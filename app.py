...
def run_macro(selected_monitor_index, working_text, warning_text, status_container):
    ...
    if stop_event.is_set():
        st.warning("프로그램이 사용자에 의해 중단되었습니다.")
        return
    ...
    try:
        ...
        while scroll_count < st.session_state.max_scrolls and items_found:
            try:
                ...
                screenshot_path = os.path.join(temp_dir, 
                    f"screenshot_{timestamp}_scroll{scroll_count}.png")
                ...
                # 디버그 이미지 저장 (매칭 결과 포함)
                debug_path = os.path.join(temp_dir, 
                    f"debug_visualization_{timestamp}_scroll{scroll_count}_matches{match_count}.png")
                cv2.imwrite(debug_path, debug_image)
                
                # 사이드바 업데이트를 위해 show_debug_images 호출
                show_debug_images()
...
def show_debug_images():
    """temp 폴더의 디버그 이미지들을 사이드바에 표시"""
    temp_dir = "./temp"
    if os.path.exists(temp_dir):
        # 스크린샷과 디버그 이미지를 분리하여 정렬
        screenshots = [f for f in os.listdir(temp_dir) if f.startswith("screenshot_")]
        debug_files = [f for f in os.listdir(temp_dir) if f.startswith("debug_visualization_")]
        
        screenshots.sort(reverse=True)  # 최신 파일이 위로 오도록
        debug_files.sort(reverse=True)
        
        if screenshots or debug_files:
            st.sidebar.markdown("---")
            st.sidebar.header("실시간 캡처 이미지")
            
            # 최신 스크린샷 표시
            if screenshots:
                latest_screenshot = screenshots[0]
                st.sidebar.subheader("현재 화면")
                st.sidebar.image(
                    os.path.join(temp_dir, latest_screenshot),
                    caption=latest_screenshot,
            ...
            if debug_files:
                ...
                st.sidebar.subheader("매칭 결과")
                st.sidebar.image(
                    os.path.join(temp_dir, latest_debug),
                    caption=latest_debug,
                    use_container_width=True
                )

# Streamlit UI
def main():
    # 시작할 때 stop_event 초기화
    stop_event.clear()
